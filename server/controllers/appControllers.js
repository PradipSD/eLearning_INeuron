...
const Material = require('../models/material'); // Ensure the model is imported
...
exports.adminLoginAPI = async (req, res) => {
  try {
    ...
    if (admin && admin.password === password) {
      const infoErrorsObj = req.flash("infoErrors");
      const infoSubmitObj = req.flash("infoSubmit");
      res.status(200).render("materialRegister", { infoErrorsObj, infoSubmitObj, materials: [] }); // Assuming you pass an empty array for materials initially
    } else {
      req.flash("infoErrors", "Invalid email or password");
...
//     req.flash("infoSubmit", "Material Successfully Added");
//     res.status(200).redirect("materialRegister");
//   } catch (error) {
//     req.flash("infoErrors", `error: ${error.message}`);
//     res.status(400).redirect("materialRegister");
//   }
// };
...
exports.materialRegisterAPI = async (req, res) => {
  try {
    ...
    await newMaterial.save();
    // Add logic to save the material to the database here
    req.flash("infoSubmit", "Material added successfully!");
    res.redirect("/materialRegister"); // Assuming this route renders the materialRegister.ejs template
  } catch (error) {
    req.flash("infoErrors", `Error: ${error.message}`);
